...
struct ChartContainer<Content: View>: View {
    ...
    var accessibilityLabel: String{
        switch chartType {
        case .stepBar(let average):
            "Bar chart, steps count, last 28 days, average steps per day: \(average) steps "
        case .stepWeekdaysPie:
            "Last 28 days "
        case .weightLine(let average):
            "Line chart, weight, average weight: \(average.formatted(.number.precision(.fractionLength(1)))) lbs, goal weight: 156 lbs"
        case .weightDiffBar:
